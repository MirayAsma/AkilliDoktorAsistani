...
class _AIAnalizRaporuPageState extends State<AIAnalizRaporuPage> {
  ...
  Future<void> _loadApiKey() async {
    ...
    if (key != null && key.isNotEmpty) {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'Gemini API anahtarı bulunamadı. Lütfen ayarlardan API anahtarınızı ekleyin.';
      });
      _showApiKeyDialog();
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Ham yanıtı gösterme (debug için)
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(_analysisResult['rawResponse'] ?? 'Yanıt yok'),
                  ],
                ),
              ),
              const SizedBox(height: 24),
            ...
            if (_analysisResult.containsKey('abnormalFindingsText')) ...[  
              _buildSectionTitle('1. Normal Değer Dışı Bulgular', Icons.warning_amber_rounded),
              _buildTextSection(_analysisResult['abnormalFindingsText']),
              const SizedBox(height: 24),
            ],
            
            if (_analysisResult.containsKey('diagnosesText')) ...[  
              _buildSectionTitle('2. Olası İlk 3 Tanı', Icons.psychology),
              _buildTextSection(_analysisResult['diagnosesText']),
              const SizedBox(height: 24),
            ],
            
            if (_analysisResult.containsKey('testsText')) ...[  
              _buildSectionTitle('3. Bu Aşamada Yapılması Gereken Tetkikler', Icons.science),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            '${widget.hastaVerileri['yas']} yaş, Erkek',
            style: TextStyle(
              fontSize: 16,
